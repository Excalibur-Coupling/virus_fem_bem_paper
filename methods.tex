...
\end{equation}
%
where $v$ is a test function, and $\left\langle\varphi,v\right\rangle_\Gamma = \int_\Gamma \varphi(\mathbf{x})v(\mathbf{x})\,\mathrm{d}\mathbf{x}$ and $\left\langle\varphi,v\right\rangle_{\Omega_1} = \int_{\Omega_1} \varphi(\mathbf{x})v(\mathbf{x})\,\mathrm{d}\mathbf{x}$ are the inner products on the surface and in the domain, respectively. Slightly abusing notation, the product $\left\langle \cdot, \cdot \right\rangle_{\Omega_1}$ on the right-hand side denotes duality paring.
